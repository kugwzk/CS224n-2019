...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings.source(source_padded).permute(1,0,2)  #[b, src_len, embed_size]
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        target_padded = target_padded[:-1]  #去掉<END>
        ...
        # Y is golden standard 
        Y = self.model_embeddings.target(target_padded)
        Y_list = list(torch.split(Y, 1))
        for Y_t in Y_list:  #one step
            Y_t = Y_t.squeeze(0)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t = alpha_t.unsqueeze(1)  #【b,1,src_len]
