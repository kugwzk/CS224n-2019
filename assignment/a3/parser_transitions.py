...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        ###
        self.stack = ['ROOT']
        self.buffer = sentence[:]	# = sentence 会修改sentence
        self.dependencies = []
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
    	...
    	while len(minibatch) > 0:
    		predict = model.predict(minibatch)
    		for i, predict in enumerate(predict):
    			minibatch[i].parse_step(predict)	#action as predict
    		minibatch = [instance for instance in minibatch if len(instance.buffer) > 0 or len(instance.stack) > 1]
    	unfinished_parses = unfinished_parses[batch_size:]
    
    for instance in partial_parses:
    	dependencies.append(instance.dependencies)
